...
stronger method studied in this paper is block-preserving PAM: each hybrid
actuator pair \(B_i=[\texttt{acc}_i,\texttt{sw}_i]\) is treated as an atomic
block, a weighted block graph is built from pairwise coupling scores, and a
spectral initialization is refined by 2-opt local search. Optional within-block
flips are allowed only when the pair remains adjacent.
...
segment-coupling proxy derived from the cut profile. The repository compares
local, random, bad-split, opposite-pair, greedy PAM, spectral PAM,
block-preserving PAM, free PAM, sensitivity-weighted PAM, rank-aware PAM, and
hybrid objectives under one manifest-driven runner.
